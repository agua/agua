...
sub getStagesByWorkflow {
	...
	my $required = ["username", "project", "workflow", "workflownumber"];
...
$where};
	...
	if ( not defined $stages or scalar(@$stages) == 0 ) {
		...
		$self->notifyError($data, "Could not insert stage $data->{name} into workflow $data->{workflow}") and return if not $success;
...
$where};
	...
	my $success = $self->_addStage($data);
	$self->logError("Could not _addStage $data->{name} into workflow $data->{workflow}") and return 0 if not $success;
...
=cut
	...
	$data->{workflownumber}=	$data->{number} if not defined $data->{workflownumber};

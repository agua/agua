...
sub addStageParameter {
	...
	$self->_removeStageParameter($data);

	my $success = $self->_addStageParameter($data);
	
 	$self->logError("Could not update stage parameter: $data->{name} in application $data->{appname}") if not defined $success;
	
	$data->{status}		=	"Updated stage parameter $data->{name} in application $data->{appname}";

...
class Agua::CLI::App with (Logger, Agua::CLI::Timer, Agua::CLI::Status, Agua::Common::Database) {
    use File::Path;
    use JSON;
	use Getopt::Simple;
    use Data::Dumper;
    ...
    has 'app'	    => ( isa => 'Str|Undef', is => 'rw', required => 0, documentation => q{Name of this object} );
    ...
    has 'savefields'    => ( isa => 'ArrayRef[Str]', is => 'rw', default => sub { ['name', 'number', 'owner', 'package', 'version', 'installdir', 'status', 'queued', 'started', 'completed', 'duration', 'locked', 'type', 'location', 'executor', 'envarfile', 'description', 'notes', 'submit', 'url', 'linkurl', 'localonly', 'stdoutfile', 'stderrfile', 'stagepid', 'stagejobid', 'workflowpid'] } );
    has 'exportfields'    => ( isa => 'ArrayRef[Str]', is => 'rw', default => sub { ['name', 'number', 'owner', 'package', 'version', 'installdir', 'status', 'submit', 'type', 'location', 'executor', 'envarfile', 'description', 'notes', 'url', 'linkurl', 'localonly', 'stdoutfile', 'stderrfile', 'queued', 'started', 'completed', 'duration', 'stagepid', 'stagejobid', 'workflowpid'] } );
    has 'appfields'    => ( isa => 'ArrayRef[Str]', is => 'rw', default => sub { ['name', 'number', 'owner', 'package', 'version', 'installdir', 'type', 'location', 'executor', 'envarfile', 'description', 'notes', 'url', 'linkurl', 'localonly'] } );
...
method addParam {
	...
	$outputfile			=	"$outputdir/$name.app" if not defined $outputfile or $outputfile eq "";

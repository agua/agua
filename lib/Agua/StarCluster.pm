...
method stopCluster {
	...
	return 0 if not $self->terminateCluster(undef, undef);
...
#### GENERATE A KEYPAIR USING PRIVATE AND PUBLIC KEYS
	...
	#### DELETE KEYPAIR
	my $success	=	$self->deleteKeypair($privatekey, $publiccert, $keyname);
	...
	#### ADD KEYPAIR
	$success	=	$self->addKeypair($privatekey, $publiccert, $keyname, $keypairfile);
...
id_rsa-$keyname  2>&1 	> $errorfile};
	...
	$self->logDebug("Problem deleting keypair from EC2: $output") and return 0 if $output ne '';
	$self->logDebug("Completed deleting keypair from EC2") and return 1 if $output eq '';
}
...
1> $keypairfile 2> $errorfile
	...
	$self->logError("Problem adding keypair to EC2: $output") and return 0 if $output ne '';
	$self->logDebug("Completed add keypair to EC2") and return 1 if $output eq '';
}

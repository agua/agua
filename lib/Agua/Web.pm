...
class Agua::Web	with (Agua::Common::Logger, Agua::Web::Https, Agua::Web::Exchange) {
...
=cut 
	...
	my $file = "/etc/fstab";
	open(FILE, $file) or die "Agua::Web::enableReboot    Can't open file: $file\n";
	my @lines = <FILE>;
	...
	open(OUT, ">$file") or die "Agua::Web::enableReboot    Can't open file: $file\n";
...
method linkDirectories {
    ...
    $self->logDebug("Agua::Web::linkDirectories(linksfile)");
...
method setPermissions {
    ...
    $self->logDebug("Agua::Web::setPermissions(permissionsfile)");
...
Agua web installation completed.
	
Browse to your new Agua instance here: 
...
If Agua is installed on a cloud server, you must enable HTTPS access:
    
    In an Amazon Web Services, you can use ec2-api tools:
...
method fileContents ($file) {
    $self->logDebug("Agua::Web::fileContents(file)");
    $self->logDebug("file", $file);
    die("Agua::Web::contents    file not defined\n") if not defined $file;
    die("Agua::Web::contents    Can't find file: $file\n$!") if not -f $file;
...
method runCommands ($commands) {
    $self->logDebug("Agua::Web::runCommands(commands)");
    foreach my $command ( @$commands )
    {
    	$self->logDebug("command", $command);		
